...
const loginCliente = async () => {
  try {
    ...
    const { data: userData, error: userError } = await supabase
      ...
      .select('email_auth, password')
      .eq('cpf', cleanedCpf)
      .single()

    if (userError) {
      console.log("Erro ao buscar usuário:", userError);
      useToastify("Erro ao buscar Usuário!", {
        autoClose: 2000,
        theme: "light",
        type: "error",
    ...
    // if (!userData) {
    //   useToastify("CPF não cadastrado!", {
    //     autoClose: 2000,
    //     theme: "light",
    //     type: "error",
    //     position: ToastifyOption.POSITION.TOP_RIGHT,
...
<template>
  <div class="auth-container">
    <div class="auth-card">
      ...
      <!-- Formulário de Login -->
      <form v-if="false" @submit.prevent="handleLogin" class="auth-form">
        <h2>Login</h2>
      ...
      <form v-if="true" @submit.prevent="loginCliente" class="auth-form">
        <h2>Login Cliente</h2>
